...
CollapsingDEResults_OneResultPerGene<-function(GSE_ID, DE_Results_GoodAnnotation, ComparisonsOfInterest, NamesOfFoldChangeColumns, NamesOfTstatColumns, CurrentResultSet){
  ...
  dir.create(paste("./", GSE_ID, "_", CurrentResultSet, sep=""))
  
  #And then sets the working directory to be that folder:
  setwd(paste("./", GSE_ID,  "_", CurrentResultSet, sep=""))
  ...
  assign(paste("DEResults", GSE_ID, CurrentResultSet, sep="_"), TempMasterResults, envir = as.environment(1))
  
  #Letting us know what that name is:
  print(paste("Output: Named DEResults", GSE_ID, CurrentResultSet, sep="_"))
